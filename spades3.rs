/* Version du programme où les cartes sont des nombres et avec la nouvelle preuve de shuffle*/
...
/* struct pour la partie de l'argument de mélange de la preuve de Groth */
struct Arg {
    c : RistrettoPoint,
    c_d : RistrettoPoint,
...
fn rand(ch : &mut Vec<Chiffre>, nums : &Vec<usize>, r : Vec<Scalar>, zk_r : &mut Vec<Zkrand>, g : &RistrettoPoint, pkc : &RistrettoPoint) {
...
// Fonction qui choisit aléatoirement des générateurs pour les commitment de la preuve rand
...
// Fonction qui calcule des vecteurs commitment de Pedersen
...
fn zkrand(che : &Vec<Chiffre>, chs : &Vec<Chiffre>, nums : &Vec<usize>, ra : &Vec<Scalar>, g : &RistrettoPoint, pkc : &RistrettoPoint) -> Zkrand {
    let n = nums.len();
    let mut rng = OsRng;
    ...
    let mut s = Vec::new();
    for i in 0..n {
        let x = Chiffre {x: che[i].x , y: che[i].y};
        let y = Chiffre {x: chs[i].x , y: chs[i].y};
        e.push(x);
        s.push(y);
    }
    let (a,b) = setup(&n);
    ...
    let zkr = Zkrand{c : c, cd : cd, ed_x : ed_x, ed_y : ed_y, fi : f, z : z, m : m, arg : arg, a : a, b : b, g : *g, pkc : *pkc,  e : e, s : s};
    zkr
}

/* Fonction qui effectue la partie arg preuve rand jusqu'au moment des calculs du vérifieur */
...
fn dec(ch : &Vec<Chiffre>, sk : &Scalar, deck : &Vec<u32>, n : usize, g : &RistrettoPoint) -> u32 {
    let carte = ch[n].y - (sk * ch[n].x);
    let mut valeur = 0u32;
    for k in 0..52 {
        let nb = deck[k];
        if carte == g * Scalar::from(nb){
            valeur = nb;
            break;
        }
    }
...
fn get_hand(deck : &Vec<u32>, ch : &Vec<Chiffre>, sk : &Scalar, n : usize, g : &RistrettoPoint) {
    let mut decksor : Vec<u32> = Vec::new();
    for i in 13*n..13*(n+1) {
        decksor.push(dec(&ch,&sk,&deck,i,&g));
    }
    println!("Carte Joueur n°{} :",n+1);
    print!("{:?}",decksor);
    println!();
}

/* Fonction de jeu où à chaque exécution une carte est jouée */
fn play(n : &usize, sk : &Scalar, pk : &Vec<RistrettoPoint>, chiffre : &Vec<Chiffre>, state : &State, state2 : &mut State, deck : &Vec<u32>, tour : &usize, g : &RistrettoPoint, jt : &mut Vec<usize>) -> (usize,u32,Zkpi0,Vec<Zkpij>,Vec<usize>) {
    let mut rng = thread_rng();
...
fn zkpi0(id : &u32, pk : &RistrettoPoint, g : &RistrettoPoint, ch : &Chiffre, sk : &Scalar) -> Zkpi0 {
    let mut rng = OsRng;
    let r : Scalar = Scalar::random(&mut rng);
    let t1 = r * g;
    let t2 = r * ch.x;
...
fn veripi0(zk : Zkpi0) -> u8 {
    let pk2 = zk.ch.y - Scalar::from(zk.id) * zk.g;
...
fn zkpij(deck : &Vec<u32>, l : &Vec<usize>, sk : &Scalar, pk : &RistrettoPoint, g : &RistrettoPoint, ch : &Chiffre, id : u32) -> Zkpij {
    let mut rng = OsRng;
    ...
    let r : Scalar = Scalar::random(&mut rng);
    for k in 0..39{
        if (deck[l[k]]/100 == id/100) && (deck[l[k]]%100 == id%100) {
            y1[k] = r * g;
            y2[k] = r * ch.x;
            indice = k;
            pk2.push(ch.y);
            ...
            pk2.push(ch.y + Scalar::from(deck[l[k]]) * g);
        }
    }
...
/* Fonction qui permet au vérifieur de vérifier si la preuve rand est accepter */ 
fn veripij(pij : &Zkpij, deck : &Vec<u32>, l : &Vec<usize>) -> u8 {
    let mut s = Scalar::zero();
    for i in 0..39 {
    ...
    for i in 0..39{
        let pk2 = pij.pk2[i] - Scalar::from(deck[l[i]]) * pij.g1;
...
fn verif(n : &usize, state : &State, state2 : &State, deck : &Vec<u32>, tour : &usize, t : &usize, carte : &u32, pi0 : Zkpi0, pij : &Vec<Zkpij>, l : &Vec<usize>, jt : &mut Vec<usize>) -> u8 {
    ...
    if (state.alpha == 4||state.alpha == 0) && (state2.alpha != 1||state2.suit != carte/100) {
        return 0;
    }
...
fn main() {
    ...
    for i in 0..52 {
        let c : RistrettoPoint = pkc + g * Scalar::from(deck[i]);
    ...
    let mut state2 = State {alpha : 4, suit : 0, u : vec! [vec![53; 13];4]};
    let mut pre = 0;
    for tour in 0..13 {
        ...
        for n in 0..4 {
            ...
            let mut jt : Vec<usize> = (13*pre..13*(pre+1)).collect();
            let mut veri = 0;
            while veri == 0 {
